  ...
  function fillSelect(id,arr){ const s=document.getElementById(id); s.innerHTML='<option value="">— pilih —</option>';
  ...
  function addRuangChip(v){ const box=document.getElementById('ruangChips'); if([...box.children].some(c=>c.dataset.v===v)) return;
  ...
  function addRuangNew(e){ if(e.key!=='Enter') return; e.preventDefault(); const v=e.target.value.trim(); if(!v) return;
  ...
  function fillMitra(){ const s=document.getElementById('mitraSelect'); s.innerHTML='<option value="">— pilih mitra —</option>';
  ...
  function setMode(m){ MODE=m; document.getElementById('modeBaru').classList.toggle('active',m==='Baru');
  ...
  function prefillMitra(){ const id=document.getElementById('mitraSelect').value; const m=(DATA.mitra||[]).find(x=>x.id===id); if(!m) return;
  ...
  function fillInduk(idMitra){ const s=document.getElementById('dokumenInduk'); s.innerHTML='<option value="">— tidak ada / dokumen ini berdiri sendiri —</option>';
  ...
  function checkDup(){ const box=document.getElementById('mitraSuggest'); const v=document.getElementById('namaMitra').value.trim(); box.innerHTML=''; if(v.length<4) return;
  ...
  function hitungBerakhir(){ const mulai=document.getElementById('tanggalMulai').value, masa=parseInt(document.getElementById('masaBerlaku').value,10), tb=document.getElementById('tanggalBerakhir');
  ...
  document.getElementById('form').addEventListener('submit', async (e)=>{
    ...
    const payload={ action:'submitKerjasama', jenisEntri:MODE,
    ...
    try{
      ...
      else if(res.status==='success'){ overlay(false);
